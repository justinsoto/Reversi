...
class Othello(Board):
    ...
    def run(self):
        ...
        Parameters: self
        Returns: nothing
        Does: Starts the game, alternates back and forth between
                the two players until the game is over.
        '''
    ...
    def play(self, x, y):
        ''' Method: play
            ...
            Returns: nothing
            Does: Alternately handles moves for both players.
                    Gets the player's move by their click on the screen
    ...
    def switch_player_turn(self):
        ''' Method: switch_player_turn
            Parameters: self
            Returns: nothing
            Does: Switches the turn to the other player.
        '''
    ...
    def report_result(self):
        ''' Method: report_result
            ...
            Does: Announces the winner and reports the final number of
                  tiles for both players.
        '''
        print('GAME OVER!!')
        if self.num_tiles[0] > self.num_tiles[1]:
            print(f'Player 1 WINS!! Player 1 has {self.num_tiles[0]} tiles, '
                  f'but Player 2 only has {self.num_tiles[1]}!')
        elif self.num_tiles[0] < self.num_tiles[1]:
            print(f'Player 2 WINS... Player 2 has {self.num_tiles[1]} tiles, '
                  f'but Player 1 only has {self.num_tiles[0]} :(')
        else:
            print("IT'S A TIE!! Both players have {self.num_tiles[0]} tiles.")
