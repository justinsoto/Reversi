...
class ai():
    ...
    def choose_move(self):
        ...
        """
        return self.strategy.choose_move(self.game)

# NOTE:
# Using triple quotes is leading to unexpected behavior (due to there already being triple-quoted strings.)
# For now, just use Visual Studio Code's batch comment/uncomment feature:
#   - To comment out a block of code: Select the lines, then press Ctrl + / (or Cmd + / on macOS). This will prepend each selected line with a #, commenting them out.
#   - To uncomment:                   Select the commented lines and press Ctrl + / (or Cmd + / on macOS) again. This will remove the leading # from each line, reactivating the code.
